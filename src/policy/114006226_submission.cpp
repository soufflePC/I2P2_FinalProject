...
constexpr int MVV_LVA_VALUES[7] = {0, 20, 60, 70, 80, 200, 1000};
...
int eval_impl(
    ...
    for(const auto& action : moves){
        ...
        if(first_move){
            ...
            score = eval_impl(next, depth - 1, history, ply + 1, ctx, p, alpha, alpha + 1);
            if(score > alpha && score < beta && !ctx.stop){
                score = eval_impl(next, depth - 1, history, ply + 1, ctx, p, alpha, beta);
            }
        }else{
            score = -eval_impl(next, depth - 1, history, ply + 1, ctx, p, -alpha - 1, -alpha);
            if(score > alpha && score < beta && !ctx.stop){
                score = -eval_impl(next, depth - 1, history, ply + 1, ctx, p, -beta, -alpha);
            }
        }
        delete next;
...
SearchResult MiniMax::search(
    ...
    for(auto& action : moves){
        ...
        if(first_move){
            score = same_player
                ? eval_impl(next, depth - 1, history, 1, ctx, p, M_MAX, P_MAX)
                : -eval_impl(next, depth - 1, history, 1, ctx, p, M_MAX, P_MAX);
            first_move = false;
        }else if(same_player){
            score = eval_impl(next, depth - 1, history, 1, ctx, p, best_score, best_score + 1);
            if(score > best_score && score < P_MAX && !ctx.stop){
                score = eval_impl(next, depth - 1, history, 1, ctx, p, best_score, P_MAX);
            }
        }else{
            score = -eval_impl(next, depth - 1, history, 1, ctx, p, -best_score - 1, -best_score);
            if(score > best_score && score < P_MAX && !ctx.stop){
                score = -eval_impl(next, depth - 1, history, 1, ctx, p, M_MAX, -best_score);
            }
        }
        delete next;
